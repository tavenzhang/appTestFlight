...
import {width, height, indexBgColor, Size} from './src/assets/resouce/theme'
import StartUpHelper from './StartUpHelper'
import AppConfig from './AppConfig'
import create from './Api'
import {RootStack} from './src/shell/MainScreen';
...
export default class App extends Component<Props> {
    ...
    componentWillMount() {
        ...
        this.getAPPVersion()
        SplashScreen.hide();
        if(NativeModules.JDHelper.enableIQKeyboard){
            NativeModules.JDHelper.enableIQKeyboard(true);
        }
    ...
    //使用从服务器获取的更新地址更新app
    gotoUpdate() {
        JXLog("AsyncStorage.getItem cacheDomain =====start")
        AsyncStorage.getItem('cacheDomain').then((response) => {
            ...
            JXLog("AsyncStorage.getItem cacheDomain =====end",cacheDomain)
        })
    }

    storeLog(message) {
        JXLog("uploadLog =====",message)
        AsyncStorage.mergeItem('uploadLog', JSON.stringify(message))
    }

    uploadLog() {
        AsyncStorage.getItem('uploadLog').then((response) => {
            JXLog("uploadLog =====",response)
            if (response != null) {
    ...
    hotFix(hotfixDeploymentKey) {
        ...
        CodePush.checkForUpdate(hotfixDeploymentKey).then((update) => {
            JXLog("checking update=="+update, update==null)
            if (update !== null) {
                if (Platform.OS == 'ios') {
                   // enableIQKeyboard
