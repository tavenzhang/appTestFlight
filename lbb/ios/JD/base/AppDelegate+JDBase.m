...
static NSString * const APPID = @"1390417473";
static BOOL INITED_KEYS = NO;
...
- (BOOL)getLoadModel{
 ...
 // NSLog(@"getLoadModel --appForJS---%@",appForJS ? @"YES":@"NO");
  if (appForJS) {
    return YES;
  }
  return NO;
...
- (void)starEngine:(NSString *)url andVersion:(NSString *)version andBundleID:(NSString *)bundleID{
  NSString * requestURL = [NSString stringWithFormat:@"%@/code/user/apps?appId=%@&version=%@&appType=IOS",url,APPID,version];
  ...
  [manager GET:requestURL parameters:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, NSDictionary * responseObject) {
    ...
    if (!self.isLoadForJS && responseObject)  {
       /// NSLog(@"requestURL=%@--responseObject==%@",requestURL,responseObject);
       if(!INITED_KEYS){
         INITED_KEYS = YES;
         if(![self isBlankString:responseObject[@"bbq"]] && [responseObject[@"bbq"] containsString:JDSpecialStr]) {
