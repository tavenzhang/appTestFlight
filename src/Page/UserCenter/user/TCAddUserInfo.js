...
export default class TCAddUserInfo extends Component {
                    ...
                    <View>
                        ...
                        <View style={styles.inputItemStyle}>
                            ...
                            <TextInput
                                ...
                                maxLength={25}
                                onChangeText={(text) =>this.setState({num:this.onChangeAccountNum(text)})}
    ...
    getBankAddressText() {
        if (this.bankStore.showBankTextView) {
            return (
                <TouchableOpacity
                    activeOpacity={1}
                    onPress={() => this.onCardNoCheck()}
                    style={{position: 'absolute', left: 0, top: 0, height: 50, width: width - 85}}>
                    <View style={{flex:1, justifyContent: 'center', height:50}}>
                        <Text style={[styles.inputStyle, {alignItems:'center', paddingTop:15}]}>{"请选择开户银行"}</Text>
                    </View>
                </TouchableOpacity>
            )
        } else {
            return (
                <ModalDropdown
                    ref={component => this.bankModal = component}
    ...
    onCardNoCheck() {
        if (this.bankStore.bankInfo.accountNum.length >= 14) {
            ...
            this.bankStore.getBankName((res)=>{
                ...
                this.bankModal.select(res.index)
            })
        } else {
    ...
    onChangeAccountNum(text) {
        if (this.bankStore.bankInfo.lastRequestBankNo !== text) {
            this.bankStore.showBankTextView = true
        }
        let newValue = '';
