...
let config = {
    ...
    dxdsNumbersArray:['大','小','单','双','龙','虎'],
    playInfo: [''],

    PlayType: ['前一', '前二', '前三', '定位胆','第一名','第二名','第三名','第四名','第五名','第六名','第七名','第八名','第九名','第十名','冠亚和值','冠亚和'],
...
export default class SingletonXYFTDPS {
    // 构造
    constructor() {
        if (!instance) {
            instance = this
    ...
    resetPlayMath(mathType) {
        ...
        amount = 0
        myMathType = mathType
        myMathTypeID = PlayMathConfig.getGameMathKeyWithTitle(myMathType,'HF_XYFT')
    }
    ...
    addNumbersDicToArray(odds, price, rebates) {
        ...
        if (numbersDic) {
            ...
            if (aStr.length > 0) {
                json.betString = aStr
                json.gameplayMethod = PlayMathConfig.getGameMathKeyWithTitle(myMathType,'HF_XYFT')
