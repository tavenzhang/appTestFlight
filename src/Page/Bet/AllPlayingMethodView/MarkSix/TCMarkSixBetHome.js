...
export default class TCMarkSixBetHome extends React.Component {
    ...
    choosePlayType(parentIndex, itemIndex) {
        ...
        navBar.setTitle(platMath.replace('平特一肖尾数-', '平特').replace('自选不中-自选不中', '自选不中'))
    ...
    showBetSettingModal() {
        ...
        betSettingView._setModalVisibleAndOddsAndMaxRebateAndNumberOfUnits(true, 0, 0, SingletonDPS.getUnAddedAllUnits())
    }

    betSetEndingEvent(json) {
        SingletonDPS.addToBetArray(json.odds, 2, json.rebate)
    ...
    pushToBetBill() {
        this.clearSelectedNumbers()
        NavigatorHelper.pushToBetBill(this.props.title, 'MarkSix', this.currentResultData.resultsData, this.props.gameUniqueId)
        this.refs['contentScrollView'].scrollTo({x: 0, y: 0, animated: false})
    }
