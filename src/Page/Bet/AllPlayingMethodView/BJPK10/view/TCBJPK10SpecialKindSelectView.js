...
//特码种类
export default class TCBJPK10SpecialKindSelectView extends React.Component {
    ...
    getAllNumbers() {
        ...
        if (prizeAmount && !this.props.prizeType_CUR) {
            for (let i = 0; i < prizeAmount.length; i++) {
                ...
                itemArr.push(<NumberView number={set.prizeNameForDisplay} numberEvent={this.props.numberEvent}
                                         myOdds={set.prizeAmount} key={i}
                                         areaIndex={areaIndex}/>)
            }
            return itemArr
            ...
            for (let i = 0; i < numberArray.length; i++) {
                if (this.props.prizeType_CUR == numberArray[i]) {
                    ...
                    itemArr.push(<NumberView number={numberArray[i]} numberEvent={this.props.numberEvent}
                                             myOdds={set.prizeAmount} key={i}
                                             areaIndex={areaIndex}/>)
                } else {
                    let set = prizeAmount[1]
                    itemArr.push(<NumberView number={numberArray[i]} numberEvent={this.props.numberEvent} key={i}
                                             areaIndex={areaIndex}
            ...
            for (let i = 0; i < numberArray.length; i++) {
                if (this.props.oddsArray) {
                    itemArr.push(<NumberView number={numberArray[i]} numberEvent={this.props.numberEvent}
                                             myOdds={this.props.oddsArray[i]} key={i}
                                             areaIndex={areaIndex}/>)
                } else {
                    itemArr.push(<NumberView number={numberArray[i]} numberEvent={this.props.numberEvent} key={i}
                                             areaIndex={areaIndex}
...
class NumberView extends React.Component {
    ...
    componentDidMount() {
        ...
        this.listener2 = RCTDeviceEventEmitter.addListener('randomSelectedNumber', (areaIndex, number, isQDXDSQ) => {
            if (this.props.areaIndex == areaIndex && this.props.number == number) {
                ...
                //          RCTDeviceEventEmitter.emit('TCMarkSixSelectView_clearView_numberSelected', this.props.areaIndex, this.props.number, true);
            }
        })
    }
